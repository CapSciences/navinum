<div class="module_search">
  <div class="filter_box" data-load-url="<?php echo url_for(array('module' => 'sf_guard_user', 'action' => 'showFilters')) ?>"></div>
                 <a class="advance_search f_left" title="<?php echo __('Recherche avancée') ?>"></a>
            
  <?php    
    $currentSearch = $sf_user->getAttribute('sf_guard_user'.'.search', '', 'admin_module');
    ...
    printf('<input id="module_search_input" class="ui-corner-left  f_left" type="text" title="%s" value="%s" name="search"/>',
      __('Rechercher dans ') . 'sf_guard_user',
      $currentSearch
    );
...
$(document).ready(function() {
	$("#module_search_input").autocomplete({
        source: '<?php echo str_replace('action','',url_for('sf_guard_user/autocomplete'))?>'
      });
});
</script>